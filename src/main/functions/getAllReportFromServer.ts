...
async function getReportsFromServer(identifier: string): Promise<ReportDB[]> {
  ...
  const temp = await fetch(`${import.meta.env.MAIN_VITE_API}/reports/all/${identifier}`, options)
...
async function getAllReportsFromServer(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  if (settings && serviceMonth && serviceMonth.status !== 'DONE') {
    ...
    getReportsFromServer(settings.identifier).then(async (reports) => {
      ...
      if (serviceMonth._id && reports.length > 0) {
        // Update serviceMonth
        ...
        new Notification({
          title: 'SECRETARY',
          body:  i18n.t('reports.forceUpdated.body', { count: reports.length }),
        }).show()
      }
    })
  }
...
export default getAllReportsFromServer
