...
import { ServiceMonthModel } from '../../types/models'
...
const firstAndLast = (arr: number[]): string => {
...
export default async function exportS88(
  ...
  let meetingAttendanceExport: MeetingAttendanceExport[] = []
  ...
  for await (const sy of serviceYears) {
    await serviceYearService.findByServiceYear(sy).then(async (serviceYear) => {
      if (serviceYear) {
        ...
        let temp: MeetingAttendanceExport = { serviceYear: sy, serviceMonths: serviceMonths }
  ...
  try {
    ...
    for (let index = 0; index < meetingAttendanceExport.length; index += 2) {
      await generateS88(
        ...
        let page = await PDFDocument.load(pdfBytes)
