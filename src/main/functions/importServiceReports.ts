...
import { ServiceMonthService } from '../../types/type'
import { ImportedReport } from '../../types/models'
          ...
          worksheet.eachRow((row, rowNumber) => {
            ...
            if (rowNumber > 1) {
              ...
              report.hasBeenInService =
                row.getCell('B').value === i18n.t('label.yes') ? true : false
              report.hasNotBeenInService =
                row.getCell('B').value === i18n.t('label.no') ? true : false
              report.auxiliary = row.getCell('D').value === i18n.t('label.yes') ? true : false
              report.studies = parseInt(row.getCell('C').value?.toString() || '') || undefined
              report.hours = parseInt(row.getCell('E').value?.toString() || '') || undefined
