...
function getPublisherRows(publishers: PublisherModel[],  serviceGroups: ServiceGroupModel[], countryCode: CountryCode): string[][] {
  const rows: string[][]   = []
...
async function generate_PDF(mainWindow: BrowserWindow,  publishers: PublisherModel[],  serviceGroups: ServiceGroupModel[],  name: string): Promise<void> {
  ...
  const rows            = getPublisherRows(publishers, serviceGroups, (settings?.congregation.country || 'SE') as CountryCode)
...
export default async function ExportAddressListEmergency(
  ...
  const name = `AddressList_${sortType.toLowerCase()}_emergency_${new Date().toLocaleDateString('sv')}`
