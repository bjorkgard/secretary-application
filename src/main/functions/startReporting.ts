...
import type { Meeting, Report }   from '../databases/schemas'
...
async function startReporting(mainWindow: BrowserWindow | null,  serviceGroupService: ServiceGroupService,  serviceMonthService: ServiceMonthService,  publisherService: PublisherService,  settingsService: SettingsService,  serviceYearService: ServiceYearService,  auxiliaryService: AuxiliaryService): Promise<string> {
  const date = new Date()
  date.setDate(0)
  ...
  for (const publisher of publishers) {
    ...
    const report: Report = {
      ...
      hasNotBeenInService: publisher.status === 'INACTIVE',
