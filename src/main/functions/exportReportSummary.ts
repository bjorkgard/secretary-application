...
import { Meeting, Report } from '../../types/models'
import savePdfFile from './savePDFFile'
import { ServiceMonthService, SettingsService } from '../../types/type'

interface jsPDFWithPlugin extends jsPDF {
  autoTable: (options: UserOptions) => jsPDF
}

const inActivePublisherService = (report: Report): boolean => {
  return (
...
const inAuxiliaryService = (report: Report): boolean => {
  return report.hasBeenInService && (report.type === 'AUXILIARY' || report.auxiliary == true)
}

const inPioneerService = (report: Report): boolean => {
  return report.hasBeenInService && report.type === 'PIONEER'
}

const inSpecialPioneerService = (report: Report): boolean => {
  return report.hasBeenInService && report.type === 'SPECIALPIONEER'
}

const inMissionaryService = (report: Report): boolean => {
  return report.hasBeenInService && report.type === 'MISSIONARY'
}

const inCircuitOverseerService = (report: Report): boolean => {
