...
const getReportsFromServer = async (identifier: string): Promise<ReportDB[]> => {
...
const resetUpdates = async (reportIds: number[]): Promise<void> => {
  const options = {
    method: 'POST',
    headers: {
      Accept: 'application/json',
...
const getReportUpdates = async (mainWindow: BrowserWindow | null): Promise<void> => {
