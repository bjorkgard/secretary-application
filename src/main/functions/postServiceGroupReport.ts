...
export async function updateServiceGroupReport(settingsService: SettingsService,  publisherService: PublisherService,  serviceGroupService: ServiceGroupService,  identifier: string,  locale: string,  serviceMonth: ServiceMonthModel,  sendMail: boolean): Promise<void> {
  const publisherId: string[]               = []
  const simplePublishers: SimplePublisher[] = []
  const serviceGroups                       = await serviceGroupService.find()
  ...
  await fetch(`${import.meta.env.MAIN_VITE_API}/reports/force_update`, options)
}
